...
./docker/run.sh -- bash -c '
  ...
  cmake --build /workspace/build -j 12'
```
...
- **`LoopClosing::mnFullBAIdx` is a generation counter declared `bool`**, so a
  second interruption of a running global BA goes undetected.
