...
// ---------------------------------------------------------------------------
template <class TDescriptor, class F>
bool TextFileVocabulary<TDescriptor, F>::loadFromTextFile(
  ...
  if (m_k < 0 || m_k > 20 || m_L < 1 || m_L > 10 || scoring < 0 ||
      scoring > 5 || weighting < 0 || weighting > 3) {
    std::cerr << "Vocabulary loading failure: '" << filename
              << "' is not a valid vocabulary text file\n";
    return false;
  }
  ...
  const auto expected_nodes = static_cast<std::size_t>(
      (std::pow(static_cast<double>(m_k), static_cast<double>(m_L) + 1) - 1) /
      (m_k - 1));
  m_nodes.reserve(expected_nodes);
  m_words.reserve(static_cast<std::size_t>(
      std::pow(static_cast<double>(m_k), static_cast<double>(m_L) + 1)));
  ...
  while (std::getline(f, line)) {
    ...
    if (parent_id < 0 || static_cast<std::size_t>(parent_id) >= nid) {
      std::cerr << "Vocabulary loading failure: node " << nid
...
// ---------------------------------------------------------------------------
template <class TDescriptor, class F>
bool TextFileVocabulary<TDescriptor, F>::saveToTextFile(
