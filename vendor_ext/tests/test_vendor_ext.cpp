...
void TestTextVocabulary() {
  ...
  for (int i = 0; i < 12; ++i) training.push_back(RandomDescriptors(20, 1000 + i));
  voc.create(training);
  check(voc.size() > 0, "upstream create() still works",
        std::to_string(voc.size()) + " words");
  ...
  check(a.size() == b.size() && voc.score(a, b) > 0.999,
        "reloaded vocabulary scores identically",
  ...
  check(!missing.loadFromTextFile("/tmp/definitely_not_a_vocabulary_9f3a.txt"),
  ...
  check(trailing.loadFromTextFile("/tmp/orbslam3r_voc_trailing.txt") &&
            trailing.size() == voc.size(),
        "trailing blank lines add no node",
        std::to_string(trailing.size()) + " words");
}
...
void TestSerialization() {
  ...
  check(!buf.str().empty(), "non-intrusive serialize() is found",
  ...
  check(bow2.size() == 2 && std::abs(bow2[9] - 0.75) < 1e-12,
        "BowVector round trip");
  check(fv2.size() == 2 && fv2[2].size() == 2 && fv2[5].size() == 1,
...
void TestG2oCompat() {
  ...
  const std::vector<Eigen::Vector3d> points = {
      {0.4, 0.2, 4.0},  {-0.6, 0.3, 5.0}, {0.2, -0.5, 3.5},
      {1.0, 0.7, 6.0},  {-0.9, -0.2, 4.5}, {0.1, 0.9, 5.5},
      {0.7, -0.8, 4.2}, {-0.4, 0.6, 3.8},
  };
  int id = 2;
  ...
  for (const auto& Xw : points) {
    ...
    if (optimizer.addVertex(p)) ++point_vertices;

    for (int cam = 0; cam < 2; ++cam) {
      const Eigen::Vector3d Xc =
          cam == 0 ? Xw : Eigen::Vector3d(R1 * Xw + t1);
      ...
      e->setMeasurement(Eigen::Vector2d(fx * Xc.x() / Xc.z() + cx,
                                        fy * Xc.y() / Xc.z() + cy));
  ...
  check(point_vertices == static_cast<int>(points.size()),
        "VertexSBAPointXYZ accepted by the graph",
  ...
  gn.setAlgorithm(orbslam3r::g2o_ext::MakeGaussNewton<
...
void TestRealVocabulary(const std::string& path) {
  ...
  if (!ok) return;
  check(voc.size() > 100000, "word count",
        std::to_string(voc.size()) + " words");
  check(voc.getBranchingFactor() == 10 && voc.getDepthLevels() == 6,
        "tree shape k=10 L=6",
        "k=" + std::to_string(voc.getBranchingFactor()) +
            " L=" + std::to_string(voc.getDepthLevels()));
  ...
  check(!bow.empty() && !fv.empty(), "transform() produces a BoW vector",
        std::to_string(bow.size()) + " words, " + std::to_string(fv.size()) +
            " nodes");
...
int main(int argc, char** argv) {
  ...
  TestSerialization();
  TestG2oCompat();
  if (argc > 1) TestRealVocabulary(argv[1]);
  else std::puts("\n(pass a path to ORBvoc.txt to also test the real vocabulary)");

  std::printf("\n%s\n", failures == 0 ? "ALL CHECKS PASSED"
                                      : "FAILURES: see [FAIL] lines above");
