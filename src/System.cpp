...
namespace ORB_SLAM3
{
          ...
          mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
    {
        ...
        const bool bViewerEnabled = viewerEnv ? (std::string(viewerEnv) != "0") : bUseViewer;
        if(bViewerEnabled)
        //if(false) // TODO
        {
    ...
    {
        std::cout << std::endl
                  << "Saving trajectory of map " << pMap->GetId() << " to " << filename << " ..." << std::endl;
        /*if(mSensor==MONOCULAR)
    {
        cerr << "ERROR: SaveTrajectoryEuRoC cannot be used for monocular." << endl;
        return;
    ...
    /*void System::SaveTrajectoryKITTI(const string &filename)
{
    ...
    void System::SaveDebugData(const int &initIdx)
    {
        ...
        f << std::fixed;
        f << mpLocalMapper->mRwg(0, 0) << "," << mpLocalMapper->mRwg(0, 1) << "," << mpLocalMapper->mRwg(0, 2)
          << std::endl;
        f << mpLocalMapper->mRwg(1, 0) << "," << mpLocalMapper->mRwg(1, 1) << "," << mpLocalMapper->mRwg(1, 2)
          << std::endl;
        f << mpLocalMapper->mRwg(2, 0) << "," << mpLocalMapper->mRwg(2, 1) << "," << mpLocalMapper->mRwg(2, 2)
          << std::endl;
        f.close();
