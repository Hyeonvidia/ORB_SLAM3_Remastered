                                   ...
                                   const bool bFarPoints, const float thFarPoints)
{
    ...
    for(size_t iMP = 0; iMP < vpMapPoints.size(); iMP++)
    {
        ...
        if(pMP->mbTrackInView)
        {
            ...
            const std::vector<size_t> vIndices = F.GetFeaturesInArea(pMP->mTrackProjX, pMP->mTrackProjY,
        ...
        if(F.Nleft != -1 && pMP->mbTrackInViewR)
        {
            ...
            if(nPredictedLevel != -1)
            {
                ...
                const std::vector<size_t> vIndices = F.GetFeaturesInArea(pMP->mTrackProjXR, pMP->mTrackProjYR,
                                                                         r * F.mvScaleFactors[nPredictedLevel],
