...
namespace ORB_SLAM3
{
    ...
    void Tracking::Track()
    {
        ...
        std::lock_guard<std::mutex> lock(pCurrentMap->mMutexMapUpdate);
            ...
            if(!vDepthIdx.empty())
            {
                ...
                for(size_t j = 0; j < vDepthIdx.size(); j++)
                {
                    ...
                    if(bCreateNew)
                    {
                        ...
                        {
                            mCurrentFrame
                                .mvpMapPoints[mCurrentFrame.Nleft + mCurrentFrame.mvLeftToRightMatch[i]] = pNewMP;
