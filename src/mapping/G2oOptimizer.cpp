...
void G2oOptimizer::GlobalBundleAdjustemnt(Map* pMap, int nIterations, bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
{
...
void G2oOptimizer::BundleAdjustment(const std::vector<KeyFrame *> &vpKFs, const std::vector<MapPoint *> &vpMP,
...
void G2oOptimizer::FullInertialBA(Map *pMap, int its, const bool bFixLocal, const long unsigned int nLoopId, bool *pbStopFlag, bool bInit, float priorG, float priorA, Eigen::VectorXd *vSingVal, bool *bHess)
{
...
int G2oOptimizer::PoseOptimization(Frame *pFrame)
{
    g2o::SparseOptimizer optimizer;
    g2o::BlockSolver_6_3::LinearSolverType * linearSolver;
...
void G2oOptimizer::LocalBundleAdjustment(KeyFrame *pKF, bool* pbStopFlag, Map* pMap, int& num_fixedKF, int& num_OptKF, int& num_MPs, int& num_edges)
{
...
{   
    ...
    for(size_t i=0, iend=vpKFs.size(); i<iend;i++)
    {
        ...
        {
            vScw[nIDi] = Converter::toG2oSim3(it->second);
            VSim3->setEstimate(Converter::toG2oSim3(it->second));
        }
        else
        {
    ...
    for(size_t i=0, iend=vpKFs.size(); i<iend; i++)
    {
        ...
        if(iti!=NonCorrectedSim3.end())
            Swi = Converter::toG2oSim3(iti->second).inverse();
        else
        ...
        if(pParentKF)
        {
            ...
            if(itj!=NonCorrectedSim3.end())
                Sjw = Converter::toG2oSim3(itj->second);
            else
                Sjw = vScw[nIDj];
        ...
        for(std::set<KeyFrame*>::const_iterator sit=sLoopEdges.begin(), send=sLoopEdges.end(); sit!=send; sit++)
        {
            ...
            if(pLKF->mnId<pKF->mnId)
            {
                ...
                if(itl!=NonCorrectedSim3.end())
                    Slw = Converter::toG2oSim3(itl->second);
                else
                    Slw = vScw[pLKF->mnId];
            ...
            if(pKFn && pKFn!=pParentKF && !pKF->hasChild(pKFn) /*&& !sLoopEdges.count(pKFn)*/)
            {
                if(!pKFn->isBad() && pKFn->mnId<pKF->mnId)
                {
                    ...
                    if(itn!=NonCorrectedSim3.end())
                        Snw = Converter::toG2oSim3(itn->second);
                    else
                        Snw = vScw[pKFn->mnId];
        ...
        if(pKF->bImu && pKF->mPrevKF)
        {
            ...
            if(itp!=NonCorrectedSim3.end())
                Spw = Converter::toG2oSim3(itp->second);
            else
...
void G2oOptimizer::OptimizeEssentialGraph(KeyFrame* pCurKF, std::vector<KeyFrame*> &vpFixedKFs, std::vector<KeyFrame*> &vpFixedCorrectedKFs,
...
int G2oOptimizer::OptimizeSim3(KeyFrame *pKF1, KeyFrame *pKF2, std::vector<MapPoint *> &vpMatches1, Sim3Type &g2oS12, const float th2,
                            const bool bFixScale, Eigen::Matrix<double,7,7> &mAcumHessian, const bool bAllPoints)
{
    // Convert Sim3Type to g2o::Sim3 for internal optimization
    g2o::Sim3 g2oS12_internal = Converter::toG2oSim3(g2oS12);
    ...
    vSim3->_fix_scale=bFixScale;
    vSim3->setEstimate(g2oS12_internal);
    vSim3->setId(0);
    vSim3->setFixed(false);
    ...
    g2oS12 = Converter::toSim3Type(vSim3_recov->estimate());

    return nIn;
}

void G2oOptimizer::LocalInertialBA(KeyFrame *pKF, bool *pbStopFlag, Map *pMap, int& num_fixedKF, int& num_OptKF, int& num_MPs, int& num_edges, bool bLarge, bool bRecInit)
{
    Map* pCurrentMap = pKF->GetMap();
...
void G2oOptimizer::InertialOptimization(Map *pMap, Eigen::Matrix3d &Rwg, double &scale, Eigen::Vector3d &bg, Eigen::Vector3d &ba, bool bMono, Eigen::MatrixXd  &covInertial, bool bFixedVel, bool bGauss, float priorG, float priorA)
{
...
void G2oOptimizer::InertialOptimization(Map *pMap, Eigen::Vector3d &bg, Eigen::Vector3d &ba, float priorG, float priorA)
{
...
void G2oOptimizer::InertialOptimization(Map *pMap, Eigen::Matrix3d &Rwg, double &scale)
{
    int its = 10;
    long unsigned int maxKFid = pMap->GetMaxKFid();
    const std::vector<KeyFrame*> vpKFs = pMap->GetAllKeyFrames();
...
void G2oOptimizer::LocalBundleAdjustment(KeyFrame* pMainKF,std::vector<KeyFrame*> vpAdjustKF, std::vector<KeyFrame*> vpFixedKF, bool *pbStopFlag)
{
...
void G2oOptimizer::MergeInertialBA(KeyFrame* pCurrKF, KeyFrame* pMergeKF, bool *pbStopFlag, Map *pMap, KeyFrameAndPose &corrPoses)
{
    const int Nd = 6;
    ...
    for(int i=0; i<N; i++)
    {
        ...
        corrPoses[pKFi] = Converter::toSim3Type(g2oSiw);
...
int G2oOptimizer::PoseInertialOptimizationLastKeyFrame(Frame *pFrame, bool bRecInit)
{
    g2o::SparseOptimizer optimizer;
    g2o::BlockSolverX::LinearSolverType * linearSolver;
...
int G2oOptimizer::PoseInertialOptimizationLastFrame(Frame *pFrame, bool bRecInit)
{
    g2o::SparseOptimizer optimizer;
    g2o::BlockSolverX::LinearSolverType * linearSolver;
...
void G2oOptimizer::OptimizeEssentialGraph4DoF(Map* pMap, KeyFrame* pLoopKF, KeyFrame* pCurKF,
                                       const KeyFrameAndPose &NonCorrectedSim3,
                                       const KeyFrameAndPose &CorrectedSim3,
                                       const std::map<KeyFrame *, std::set<KeyFrame *> > &LoopConnections)
{
    ...
    for(size_t i=0, iend=vpKFs.size(); i<iend;i++)
    {
        ...
        {
            vScw[nIDi] = Converter::toG2oSim3(it->second);
            const g2o::Sim3 Swc = Converter::toG2oSim3(it->second).inverse();
    ...
    for(size_t i=0, iend=vpKFs.size(); i<iend; i++)
    {
        ...
        if(iti!=NonCorrectedSim3.end())
            Siw = Converter::toG2oSim3(iti->second);
        else
            Siw = vScw[nIDi];
        ...
        if(pParentKF)
        {
            ...
            if(itj!=NonCorrectedSim3.end())
                Swj = Converter::toG2oSim3(itj->second).inverse();
            else
        ...
        for(std::set<KeyFrame*>::const_iterator sit=sLoopEdges.begin(), send=sLoopEdges.end(); sit!=send; sit++)
        {
            ...
            if(pLKF->mnId<pKF->mnId)
            {
                ...
                if(itl!=NonCorrectedSim3.end())
                    Swl = Converter::toG2oSim3(itl->second).inverse();
                else
        ...
        for(std::vector<KeyFrame*>::const_iterator vit=vpConnectedKFs.begin(); vit!=vpConnectedKFs.end(); vit++)
        {
            ...
            if(pKFn && pKFn!=pParentKF && pKFn!=prevKF && pKFn!=pKF->mNextKF && !pKF->hasChild(pKFn) && !sLoopEdges.count(pKFn))
            {
                if(!pKFn->isBad() && pKFn->mnId<pKF->mnId)
                {
                    ...
                    if(itn!=NonCorrectedSim3.end())
                        Swn = Converter::toG2oSim3(itn->second).inverse();
                    else
