...
void LoadIMU(const std::string &strImuPath, std::vector<double> &vTimeStamps, std::vector<cv::Point3f> &vAcc,
...
int main(int argc, char** argv)
{
    ...
    if(argc < 7)
    {
        std::cerr
            << std::endl
            << "Usage: ./stereo_inertial_tum_vi path_to_vocabulary path_to_settings path_to_image_folder_1 path_to_image_folder_2 path_to_times_file path_to_imu_data (trajectory_file_name)"
            << std::endl;
        return 1;
    }
    ...
    for(seq = 0; seq < num_seq; seq++)
    {
        ...
        LoadImagesTUMVI(std::string(argv[4 * (seq + 1) - 1]), std::string(argv[4 * (seq + 1)]),
                        std::string(argv[4 * (seq + 1) + 1]), vstrImageLeftFilenames[seq], vstrImageRightFilenames[seq],
                        vTimestampsCam[seq]);
    ...
    for(seq = 0; seq < num_seq; seq++)
    {
        ...
        for(int ni = 0; ni < nImages[seq]; ni++, proccIm++)
        {
            ...
            {
                std::cerr << std::endl << "Failed to load image at: " << vstrImageLeftFilenames[seq][ni] << std::endl;
                return 1;
            }
