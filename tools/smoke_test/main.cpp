...
static void test_eigen_opencv() {
  ...
  check(m.total() == 16 && m.type() == CV_8U, "cv::Mat allocation",
        std::string("OpenCV ") + CV_VERSION);
}
...
static void test_g2o() {
  ...
  const std::vector<Eigen::Vector3d> points = {
      {0.5, 0.2, 4.0}, {-0.7, 0.4, 5.0}, {0.1, -0.6, 3.5},
      {1.2, 0.9, 6.0}, {-1.1, -0.3, 4.5}, {0.3, 0.8, 5.5},
  };
  size_t edges = 0;
  for (const auto& Xw : points) {
    ...
    if (optimizer.addEdge(e)) ++edges;
  }
  check(edges == points.size(), "EdgeSE3ProjectXYZOnlyPose",
  ...
  check(std::abs(sim3->estimate().scale() - 1.0) < 1e-12,
        "VertexSim3Expmap (types_sim3)");
}
...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i)
    if (std::strcmp(argv[i], "--gl") == 0) open_window = true;
