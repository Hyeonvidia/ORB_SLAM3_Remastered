#!/usr/bin/env bash
# Runs inside the dev container for monitor.sh's --x11 mode: start a nested X
# server, give the viewer input focus, then run the SLAM binary.
#
#   container_x11_launch.sh <log_dir> <binary_path> [slam args...]
#
# This lives in its own file rather than inline in monitor.sh's `docker run
# bash -c "..."` because the quoting there has to survive two shells, and the
# focus loop silently stopped working when it did not.
#
...
# the pointer, but the viewer also takes key presses.
(
